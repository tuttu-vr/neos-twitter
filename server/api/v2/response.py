...
def _process_message(mes: dict, blacklist: List[str] = []) -> str:
    utc_time = datetime.datetime.strptime(
        mes['created_datetime'], DATETIME_FORMAT).replace(tzinfo=TIMEZONE_UTC)
    ...
    try:
        ...
        for key in blacklist:
            del processed[key]
        result = ';'.join(
            [f'{key}={value}' for key, value in processed.items()])
        return result
    except TypeError:
...
def _process_messages(messages: List[dict]) -> List[str]:
    text_list = list(
        filter(lambda x: x, [_process_message(mes) for mes in messages]))
    return text_list
...
def _process_profile(twitter_user_raw: dict):
    ...
    result = ';'.join([
        f'{neotter_key}={quote(user_dict[key])}'
        for key, neotter_key in parameters.items()
    ])
    return result
...
def get_home_timeline(
        user: NeotterUser, from_id: str = 0, count: int = 10) -> str:
...
def get_user_timeline(user: NeotterUser, twitter_user_id: str) -> str:
    twitter_user_raw, user_timeline = twitter.get_user_timeline(
        user, twitter_user_id)
    if twitter_user_raw is None:
        logger.info('No user found by: %s %s' % (twitter_user_id, user.name))
        return 'No User Found'
    return (f'{_process_profile(twitter_user_raw)}'
            f'|{_join_to_str(user_timeline)}')
